// Volumetric Clouds Post Processing Shader
...
layout (set = 0, binding = 0) uniform sampler2D u_NoiseTexture;
...
void gatherPositions(inout positionStruct pos, vec2 texcoord)
{
	pos.texcoord = texcoord;    
...
vec3 calcAtmosphericScatter(positionStruct pos, out vec3 absorbLight){
    ...
    float lDotW = dot((vec3(-u_DirectionalLight.direction)), pos.worldVector);
    float lDotU = dot((vec3(-u_DirectionalLight.direction)), vec3(0.0, 1.0, 0.0));
...
float Get3DNoise(vec3 pos) 
{
    ...
    vec2 noise = vec2(texture(u_NoiseTexture, coord).x,
					  texture(u_NoiseTexture, coord + zStretch).x);
...
float getCloudShadow(vec3 p, positionStruct pos)
{
    ...
    float rSteps = cloudThickness / float(steps) / abs((vec3(-u_DirectionalLight.direction)).y);
    
    vec3 increment = (vec3(-u_DirectionalLight.direction)) * rSteps;
    vec3 position = (vec3(-u_DirectionalLight.direction)) * (cloudMinHeight - p.y) / (vec3(-u_DirectionalLight.direction)).y + p;
...
void main()
{
    gatherPositions(pos, v_TexCoord);
    
    float dither = bayer16(v_TexCoord);
