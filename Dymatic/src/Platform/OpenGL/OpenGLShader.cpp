...
namespace Dymatic {
		...
		: m_FilePath(filepath)
	{
...

		{
			...
			CreateProgram();
			DY_CORE_WARN("Shader creation took {0} ms", timer.ElapsedMillis());
		}
	...
	void OpenGLShader::CompileOrGetVulkanBinaries(const std::unordered_map<GLenum, std::string>& shaderSources)
	{
		...
		for (auto&& [stage, source] : shaderSources)
		{
			...
			else
			{
				...
				{
					DY_CORE_ERROR(module.GetErrorMessage());
					DY_CORE_ASSERT(false);
				}
	...
	void OpenGLShader::CreateProgram()
	{
		...
		if (isLinked == GL_FALSE)
		{
			...
			DY_CORE_ERROR("Shader linking failed ({0}):\n{1}", m_FilePath, infoLog.data());
	...
	void OpenGLShader::Reflect(GLenum stage, const std::vector<uint32_t>& shaderData)
	{
		...
		DY_CORE_TRACE("OpenGLShader::Reflect - {0} {1}", Utils::GLShaderStageToString(stage), m_FilePath);
		DY_CORE_TRACE("    {0} uniform buffers", resources.uniform_buffers.size());
		DY_CORE_TRACE("    {0} resources", resources.sampled_images.size());
