...
namespace Dymatic {
	...
	void Server::NetworkThreadFunc()
	{
		...
		DY_CORE_TRACE("Server listening on port {}", m_Port);
		...
		DY_CORE_TRACE("Closing connections...");
	...
	void Server::OnConnectionStatusChanged(SteamNetConnectionStatusChangedCallback_t* status)
	{
		...
		case k_ESteamNetworkingConnectionState_Connecting:
		{
			...
			{
				m_Interface->CloseConnection(status->m_hConn, 0, nullptr, false);
				DY_CORE_ERROR("Couldn't accept connection (it was already closed?)");
				break;
			}
			...
			{
				m_Interface->CloseConnection(status->m_hConn, 0, nullptr, false);
				DY_CORE_ERROR("Failed to set poll group");
				break;
			}
	...
	void Server::PollIncomingMessages()
	{
		...
		while (m_Running)
		{
			...
			{
				DY_CORE_ERROR("Received data from unregistered client");
				continue;
			}
	...
	{
		DY_CORE_ERROR("Server Fatal Error '{}'", message);
		m_Running = false;
	}
