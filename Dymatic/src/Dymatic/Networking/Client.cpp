...
namespace Dymatic {
	...
	void Client::OnConnectionStatusChanged(SteamNetConnectionStatusChangedCallback_t* info)
	{
		...
		case k_ESteamNetworkingConnectionState_ProblemDetectedLocally:
		{
			...
			if (info->m_eOldState == k_ESteamNetworkingConnectionState_Connecting)
			{
				...
				DY_CORE_ERROR("Could not connect to remove host. '{}'", info->m_info.m_szEndDebug);
			}
			else if (info->m_info.m_eState == k_ESteamNetworkingConnectionState_ProblemDetectedLocally)
			{
				DY_CORE_WARN("Lost connection with remote host. '{}'", info->m_info.m_szEndDebug);
			}
			else
			{
				DY_CORE_INFO("Disconnected from host. Code '{}' : '{}'", info->m_info.m_eEndReason, info->m_info.m_szEndDebug);
			}
	...
	{
		DY_CORE_ERROR("Client Fatal Error '{}'", message);
		m_Running = false;
	}
