...
void ed::Link::UpdateEndpoints()
{
    ...
    // IMGUI CUSTOM
    if (m_StartPin->m_LinkOffset != 0.0f)
    {
        const ImVec2 delta = line.B - line.A;
        const ImVec2 perp = ImVec2(-delta.y, delta.x);
        const ImVec2 offset = perp / sqrt(perp.x * perp.x + perp.y * perp.y) * m_StartPin->m_LinkOffset;
        m_Start += offset;
        m_End += offset;
    }
}
...
void ed::NodeBuilder::BeginPin(PinId pinId, PinKind kind)
{
    ...
    m_CurrentPin->m_LinkOffset  = editorStyle.PinLinkOffset;
...
float* ed::Style::GetVarFloatAddr(StyleVar idx)
{
    switch (idx)
    {
        ...
        // IMGUI CUSTOM
        case StyleVar_PinLinkOffset:            return &PinLinkOffset;
