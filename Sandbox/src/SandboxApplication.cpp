...
public:
		...
		: Layer("Example"), m_Camera(-1.6f, 1.6f, -0.9f, 0.9f), m_CameraPosition(0.0f)
	{
		...
		m_Shader.reset(Dymatic::Shader::Create(vertexSrc, fragmentSrc));
		...
		m_FlatColorShader.reset(Dymatic::Shader::Create(flatColorShaderVertexSrc, flatColorShaderFragmentSrc));
		...
		m_TextureShader.reset(Dymatic::Shader::Create(textureShaderVertexSrc, textureShaderFragmentSrc));

		m_Texture = Dymatic::Texture2D::Create("assets/textures/Checkerboard.png");
			
		std::dynamic_pointer_cast<Dymatic::OpenGLShader>(m_TextureShader)->Bind();
		std::dynamic_pointer_cast<Dymatic::OpenGLShader>(m_TextureShader)->UploadUniformInt("u_Texture", 0);
	}

	void OnUpdate(Dymatic::Timestep ts) override
	{

		if (Dymatic::Input::IsKeyPressed(DY_KEY_LEFT))
			m_CameraPosition.x -= m_CameraMoveSpeed * ts;

		else if (Dymatic::Input::IsKeyPressed(DY_KEY_RIGHT))
			m_CameraPosition.x += m_CameraMoveSpeed * ts;

		if (Dymatic::Input::IsKeyPressed(DY_KEY_UP))
			m_CameraPosition.y += m_CameraMoveSpeed * ts;

		else if (Dymatic::Input::IsKeyPressed(DY_KEY_DOWN))
			m_CameraPosition.y -= m_CameraMoveSpeed * ts;

		if (Dymatic::Input::IsKeyPressed(DY_KEY_A))
			m_CameraRotation += m_CameraRotationSpeed * ts;

		if (Dymatic::Input::IsKeyPressed(DY_KEY_D))
			m_CameraRotation -= m_CameraRotationSpeed * ts;
		...
		Dymatic::Renderer::BeginScene(m_Camera);
		...
		m_Texture->Bind();
		Dymatic::Renderer::Submit(m_TextureShader, m_SquareVA, glm::scale(glm::mat4(1.0f), glm::vec3(1.5f)));
		
		// (Triangle Code)	Dymatic::Renderer::Submit(m_Shader, m_VertexArray);
...
private:
	...
	Dymatic::Ref<Dymatic::Shader> m_FlatColorShader, m_TextureShader;
	Dymatic::Ref<Dymatic::VertexArray> m_SquareVA;

	Dymatic::Ref<Dymatic::Texture2D> m_Texture;

	Dymatic::OrthographicCamera m_Camera;
