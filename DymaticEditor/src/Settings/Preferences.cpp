...
namespace Dymatic {
	...
	void Preferences::Init()
	{
		...
		LoadKeymap("saved/presets/keymaps/DymaticDefault.keymap");
		LoadKeymap("saved/SavedKeymap.keymap");

		LoadWorkspace("saved/presets/workspaces/DefaultWorkspace.workspace");
		LoadWorkspace(s_WorkspaceSaveFilepath);
	}
	...
	std::string Preferences::GetThemeColorName(uint8_t idx)
	{
		...
		case ImGuiCol_LogTrace: return "Log Trace";
		case ImGuiCol_LogInfo: return "Log Info";
		case ImGuiCol_LogWarn: return "Log Warn";
		case ImGuiCol_LogError: return "Log Error";
		case ImGuiCol_LogCritical: return "Log Critical";
	...
	uint8_t Preferences::GetThemeColor(const std::string& name)
	{
		...
		else if (name == "Log Trace") { return ImGuiCol_LogTrace; }
		else if (name == "Log Info") { return ImGuiCol_LogInfo; }
		else if (name == "Log Warn") { return ImGuiCol_LogWarn; }
		else if (name == "Log Error") { return ImGuiCol_LogError; }
		else if (name == "Log Critical") { return ImGuiCol_LogCritical; }
	...
	void Preferences::SaveKeymap(const std::filesystem::path& filepath)
	{
		YAML::Emitter out;
		out << YAML::BeginSeq;
		for (uint32_t i = 0; i < Preferences::Keymap::BIND_EVENT_SIZE; i++)
		{
	...
	std::string Preferences::GetEditorWindowName(EditorWindow editorWindow)
	{
		switch (editorWindow)
		{
		case EditorWindow::Viewport: return "Viewport";
		case EditorWindow::Toolbar: return "Toolbar";
		case EditorWindow::Statistics: return "Statistics";
		...
		case EditorWindow::ContentBrowser: return "Content Browser";
	...
	Preferences::EditorWindow Preferences::GetEditorWindow(const std::string& name)
	{
		if (name == "Viewport") return EditorWindow::Viewport;
		if (name == "Toolbar") return EditorWindow::Toolbar;
		if (name == "Statistics") return EditorWindow::Statistics;
		...
		if (name == "Content Browser") return EditorWindow::ContentBrowser;
		...
		DY_CORE_ASSERT(false, "Editor Window does not exist.");
		return EditorWindow::None;
	}

	bool& Preferences::GetEditorWindowVisible(EditorWindow editorWindow)
	{
		return s_EditorWindowVisible[editorWindow];
	}
	
	bool Preferences::LoadWorkspace(const std::filesystem::path& filepath)
	{
		if (!std::filesystem::exists(filepath))
			return false;
		...
		try
		{
			workspace = YAML::LoadFile(filepath.string());
		}
		catch (YAML::ParserException e)
		{
			DY_CORE_ERROR("Failed to load workspace file '{}'\n     {}", filepath, e.what());
			return false;
		}

		if (workspace)
		{
			auto editorWindowVisible = workspace["Editor Window Visible"];
			if (editorWindowVisible)
			{
				for (auto window : editorWindowVisible)
				{
					EditorWindow editorWindow = GetEditorWindow(window.first.as<std::string>());
					
					if (editorWindow == None)
						continue;

					s_EditorWindowVisible[editorWindow] = window.second.as<bool>();
				}
			}

			auto iniSettings = workspace["Ini Settings"];
			if (iniSettings)
			{
				std::string iniString = iniSettings.as<std::string>();
				ImGui::LoadIniSettingsFromMemory(iniString.c_str(), iniString.length());
			}
		}
	}

	bool Preferences::LoadWorkspace()
	{
		return LoadWorkspace(s_WorkspaceSaveFilepath);
	}

	void Preferences::SaveWorkspace(const std::filesystem::path& filepath)
	{
		...
		out << YAML::Key << "Editor Window Visible" << YAML::Key << YAML::BeginMap;
		for (uint32_t i = 0; i < EDITOR_WINDOW_COUNT; i++)
		{
			std::string editorWindowName = GetEditorWindowName((EditorWindow)i);

			if (!editorWindowName.empty())
				out << YAML::Key << editorWindowName << YAML::Value << s_EditorWindowVisible[i];
		}
		out << YAML::EndMap;

		out << YAML::Key << "Ini Settings" << YAML::Value << ImGui::SaveIniSettingsToMemory();
	...
	void Preferences::SaveWorkspace()
	{
	...
	std::vector<Preferences::Keymap::KeyBindEvent> Preferences::Keymap::CheckKey(KeyPressedEvent event)
	{
		...
		for (auto& bind : s_Keymap)
		{
			...
			{
				if (((bind.Ctrl ) == (Input::IsKeyPressed(Key::LeftControl) || Input::IsKeyPressed(Key::RightControl))) &&
					((bind.Shift) == (Input::IsKeyPressed(Key::LeftShift  ) || Input::IsKeyPressed(Key::RightShift  ))) &&
					((bind.Alt  ) == (Input::IsKeyPressed(Key::LeftAlt    ) || Input::IsKeyPressed(Key::RightAlt    )))
					)
				{
	...
	std::string Preferences::Keymap::GetBindEventName(KeyBindEvent event)
	{
		...
		else if (event == VisualizationAlbedoBind) return "Visualization Albedo";
		else if (event == VisualizationNormalBind) return "Visualization Normal";
		else if (event == VisualizationEntityIDBind) return "Visualization Entity ID";
		else if (event == ToggleVisualizationBind) return "Toggle Visualization";
		...
		else if (event == OpenCommandLineBind) return "Open Command Line";
	...
	Preferences::Keymap::KeyBindEvent Preferences::Keymap::GetBindEventByName(const std::string& name)
	{
		...
		else if (name == "Visualization Albedo") return Preferences::Keymap::VisualizationAlbedoBind;
		else if (name == "Visualization Normal") return Preferences::Keymap::VisualizationNormalBind;
		else if (name == "Visualization Entity ID") return Preferences::Keymap::VisualizationEntityIDBind;
		else if (name == "Toggle Visualization") return Preferences::Keymap::ToggleVisualizationBind;
		...
		else if (name == "Open Command Line") return Preferences::Keymap::OpenCommandLineBind;
