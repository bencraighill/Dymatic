...
namespace TextEditorInternal {
	...
	public:
		...
		{
			Char mChar;
			ImGuiCol_ mColorIndex = ImGuiCol_TextEditorDefault;
			bool mComment : 1;
			bool mMultiLineComment : 1;
			bool mPreprocessor : 1;

			Glyph(Char aChar, ImGuiCol_ aColorIndex) : mChar(aChar), mColorIndex(aColorIndex),
		...
		{
			typedef std::pair<std::string, ImGuiCol_> TokenRegexString;
		...
		unsigned int GetUndoIndex();

	private:
		typedef std::vector<std::pair<std::regex, ImGuiCol_>> RegexList;
...
namespace Dymatic {
	...
	struct TextEditorInformation
	{
		TextEditorInformation(int id, std::string filepath, bool untitled = false);

		std::string Filename, Filepath, Language;
		TextEditorInternal::TextEditor textEditor;
		unsigned int ID;
		unsigned int InternalID;
		bool Untitled;
		bool SetSelected = false;
		unsigned int SavedUndoIndex = 0;
	...
	private:
		...
		void OpenTextFile();
		void SaveTextFile();
		bool SaveTextFileByReference(TextEditorInformation* reference);
		bool SaveAsTextFile();
		void SaveFileToFilePath(std::string filepath);
		bool DeleteTextEditor(TextEditorInformation* reference);

		void UpdateLanguage(TextEditorInformation* reference);
