...
public:
    ...
    // Custom Events
	Node* SpawnCustomEventNode(SpawnNodeData in);
	Node* SpawnCallCustomEventNode(SpawnNodeData in);

    // Macros
    Node* SpawnMacroNode(SpawnNodeData in);
    ...
    Node* SpawnNodeFromLibrary(int id, NodeGraph& graph);
    ...
    FunctionDeclaration* FindFunctionByID(int id);
    ...
    bool ValueTypeSelection(PinData& data, bool flow = false);
    ...
    Variable& CreateVariable(const std::string& name = "", PinType type = PinType::Void);
...
private:
    ...
    Dymatic::Ref<Dymatic::Texture2D> m_HeaderBackground;
    ImTextureID          m_SampleImage;
	Dymatic::Ref<Dymatic::Texture2D> m_SaveIcon;
	Dymatic::Ref<Dymatic::Texture2D> m_RestoreIcon;
	Dymatic::Ref<Dymatic::Texture2D> m_CommentIcon;
	Dymatic::Ref<Dymatic::Texture2D> m_PinIcon;
	Dymatic::Ref<Dymatic::Texture2D> m_PinnedIcon;

	Dymatic::Ref<Dymatic::Texture2D> m_RemoveIcon;

	Dymatic::Ref<Dymatic::Texture2D> m_GraphIcon;
	Dymatic::Ref<Dymatic::Texture2D> m_MacroIcon;

    // Node Icons
    Dymatic::Ref<Dymatic::Texture2D> m_EventIcon;
    Dymatic::Ref<Dymatic::Texture2D> m_CustomEventIcon;
    Dymatic::Ref<Dymatic::Texture2D> m_FunctionIcon;
    Dymatic::Ref<Dymatic::Texture2D> m_SequenceIcon;
    Dymatic::Ref<Dymatic::Texture2D> m_BranchIcon;
    Dymatic::Ref<Dymatic::Texture2D> m_SwitchIcon;
...
// Custom Events
Node* NodeEditorInternal::SpawnCustomEventNode(SpawnNodeData in)
{
	auto& node = (in.GraphID ? FindGraphByID(in.GraphID) : GetCurrentGraph())->Nodes.emplace_back(GetNextId(), "Custom Event", ImColor(255, 128, 128), NodeFunction::CustomEvent);
    node.DisplayName = "Custom Event";
	node.Outputs.emplace_back(GetNextId(), "", PinType::Flow);
...
Node* NodeEditorInternal::SpawnCallCustomEventNode(SpawnNodeData in)
{
    auto event = FindNode(in.ReferenceID);
    if (event != nullptr)
	{
		const std::string name = event->Name;

        auto& node = (in.GraphID ? FindGraphByID(in.GraphID) : GetCurrentGraph())->Nodes.emplace_back(GetNextId(), name, ImColor(128, 195, 248), NodeFunction::CallCustomEvent);

        // Find again after reallocation
        event = FindNode(in.ReferenceID);

        node.DisplayName = name;
        node.Inputs.emplace_back(GetNextId(), "", PinType::Flow);

		for (auto& input : event->Outputs)
			if (input.Data.Type != PinType::Flow)
				node.Inputs.emplace_back(GetNextId(), input.Name, input.Data.Type);

        node.Outputs.emplace_back(GetNextId(), "", PinType::Flow);
        node.Icon = m_CustomEventIcon;
        node.ReferenceId = in.ReferenceID;
...
// Macros
Node* NodeEditorInternal::SpawnMacroNode(SpawnNodeData in)
{
	if (auto macro = FindGraphByID(in.ReferenceID))
	{
		const std::string name = macro->Name;

		auto& node = (in.GraphID ? FindGraphByID(in.GraphID) : GetCurrentGraph())->Nodes.emplace_back(GetNextId(), name, ImColor(255, 255, 255), NodeFunction::Macro);

		// Find again after reallocation
        macro = FindGraphByID(in.ReferenceID);
...
{
	if (m_Variables.find(in.ReferenceID) == m_Variables.end())
		return nullptr;

    auto& variable = m_Variables[in.ReferenceID];
...
Node* NodeEditorInternal::SpawnNodeFromLibrary(int id, NodeGraph& graph)
{
...
void NodeEditorInternal::CompileNodes()
{
	...
	m_CompilerResults.push_back({ CompilerResultType::Info, "Build started [C++] - Dymatic Nodes V1.2.5 (" + NodeScriptName + ")" });
	...
	header.WriteLine("//Dymatic C++ Node Script - Header - V1.2.5");
    header.NewLine();

	source.WriteLine("//Dymatic C++ Node Script - Source - V1.2.5");
	source.WriteLine("#include \"" + NodeScriptName + ".dyheader\"");
	source.WriteLine("#include \"NodeLibrary.h\"");
    source.NewLine();
	...
	{
		if (!variable.second.Tooltip.empty())
			header.WriteLine("// Tooltip: " + variable.second.Tooltip);

        if (variable.second.InstanceEditable)
            header.WriteLine("DYPARAM()");

        header.WriteLine(header.ConvertPinTypeToCompiledString(variable.second.Data.Type) + " " + header.UnderscoreSpaces("bpv__" + variable.second.Name + "_" + std::to_string(variable.second.ID) + "__pf") + ";");
    }
    ...
    {
        if (!variable.second.Tooltip.empty())
            source.WriteLine("// Tooltip: " + variable.second.Tooltip);
        source.WriteLine(source.UnderscoreSpaces(("bpv__" + variable.second.Name + "_" + std::to_string(variable.second.ID) + "__pf")) + " = " + source.ConvertPinValueToCompiledString(variable.second.Data.Type, variable.second.Data.Value) + ";");
    }
    ...
    for (auto& graph : m_Graphs)
    {
        ...
        for (auto& node : graph.Nodes)
            if (node.Function == NodeFunction::Event || node.Function == NodeFunction::CustomEvent)
            {
        ...
        for (auto& ubergraph : ubergraphs)
        {
            ...
            else
            {
                ...
                for (int i = 0; i < ubergraph.Nodes.size(); i++)
                {
                    ...
                    // Event Specifics
                    else if (node.Function == NodeFunction::Event || node.Function == NodeFunction::CustomEvent)
                    {
                        if (nonLinearUbergraph)
                        {
                            if (IsPinLinked(node.Outputs[0].ID))
                    ...
                    else if (node.Function == NodeFunction::CallCustomEvent)
                    {
                        if (nonLinearUbergraph)
                        {
                            ubergraphBody.WriteLine("case " + std::to_string(node.ID.Get()) + ":");
                        ...
                        for (auto& graph : m_Graphs)
                            for (auto& event : graph.Nodes)
                                if (event.Function == NodeFunction::CustomEvent)
	...
	std::filesystem::path directory = "../../DymaticScriptingEnvironment/DymaticTestProject/NodeGenerated/";
    ...
    // Copy Required Files
    if (std::filesystem::exists(directory / "NodeCore.h")) 
        std::filesystem::remove(directory / "NodeCore.h");
    if (std::filesystem::exists(directory / "NodeLibrary.h")) 
        std::filesystem::remove(directory / "NodeLibrary.h");
    std::filesystem::copy("src/Nodes/NodeCore.h", directory / "NodeCore.h");
    std::filesystem::copy("src/Nodes/NodeLibrary.h", directory / "NodeLibrary.h");
	...
	if (errorCount == 0)
	{
		header.OutputToFile(filepath.string() + ".dyheader");
		source.OutputToFile(filepath.string() + ".cpp");
		m_CompilerResults.push_back({ CompilerResultType::Info, "Compile of " + NodeScriptName + " completed [C++] - Dymatic Nodes V1.2.5 (" + filepath.string() + ")" });
	}
	else
		m_CompilerResults.push_back({ CompilerResultType::Info, "Compile of " + NodeScriptName + " failed [C++] - " + std::to_string(errorCount) + " Error(s) " + std::to_string(warningCount) + " Warnings(s) - Dymatic Nodes V1.2.5 (" + filepath.string() + ")" });
}
...
void NodeEditorInternal::RecursivePinWrite(Pin& pin, NodeCompiler& source,NodeCompiler& header, NodeCompiler& ubergraphBody, std::string& line, std::vector<ed::NodeId>& node_errors, std::vector<ed::NodeId>& pureNodeList, std::vector<ed::NodeId>& localNodeList)
{
	if (IsPinLinked(pin.ID))
	{
        ...
        else if (otherNode->Function == NodeFunction::Event || otherNode->Function == NodeFunction::CustomEvent)
        {
            line += "b0l__NodeEvent_" + header.GenerateFunctionName(*otherNode) + "_" + header.UnderscoreSpaces(otherPin->Name) + "__pf";
        }
        else
        {
            if (otherNode->Pure)
...
void NodeEditorInternal::RecursiveUbergraphOrganisation(Node& node, std::vector<Ubergraph>& ubergraphs, int& currentId)
{
...
FunctionDeclaration* NodeEditorInternal::FindFunctionByID(int id)
{
    for (auto& library : m_NodeLibraries)
        for (auto& function : library.m_FunctionDeclarations)
            if (function.ID == id)
...
std::string NodeCompiler::GenerateFunctionSignature(Node& node, const std::string& Namespace)
{
    ...
    for (auto& pin : node.Inputs)
    {
        if (pin.Data.Type != PinType::Flow)
        {
            ...
            output += "const " + ConvertPinTypeToCompiledString(pin.Data.Type) + " " + UnderscoreSpaces(pin.Name);
        }
    }
...
NodeGraph* NodeEditorInternal::FindGraphByID(int id)
{
    if (id)
        for (auto& graph : m_Graphs)
            if (graph.ID == id)
...
GraphWindow* NodeEditorInternal::FindWindowByID(int id)
{
	if (id)
		for (auto& window : m_Windows)
			if (window.ID == id)
...
GraphWindow* NodeEditorInternal::OpenGraph(int id)
{
...
GraphWindow* NodeEditorInternal::OpenGraphInNewTab(int id)
{
    ...
    if (data.Container == ContainerType::Single && !data.Reference)
    {
        ...
        case PinType::Color: {
            ...
            if (spring)
            {
				...
				if (ImGui::ColorButton("##ColorButton", col_v4, ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_NoTooltip | ImGuiColorEditFlags_AlphaPreviewHalf))
				{
                ...
                ImGui::ColorEdit4("##NodeEditorColor", glm::value_ptr(data.Value.Color), ImGuiColorEditFlags_AlphaPreviewHalf);
            break;
        }
        case PinType::Byte: {
...
bool NodeEditorInternal::ValueTypeSelection(PinData& data, bool flow)
{
    bool return_value = false;

	auto& cpos1 = ImGui::GetCursorPos();
	if (ImGui::BeginCombo("##VariableType", ("       " + ConvertPinTypeToString(data.Type)).c_str()))
	{
        if (flow)
            if (ImGui::MenuItem("    Flow")){ data = PinType::Flow;  return_value = true; }
        if (ImGui::MenuItem("    Boolean")) {data = PinType::Bool;   return_value = true; }
		if (ImGui::MenuItem("    Integer")) {data = PinType::Int;    return_value = true; }
		if (ImGui::MenuItem("    Float"))   {data = PinType::Float;  return_value = true; }
		if (ImGui::MenuItem("    String"))  {data = PinType::String; return_value = true; }
		if (ImGui::MenuItem("    Vector"))  {data = PinType::Vector; return_value = true; }
		if (ImGui::MenuItem("    Color"))   {data = PinType::Color;  return_value = true; }
		if (ImGui::MenuItem("    Byte"))    {data = PinType::Byte;   return_value = true; }
		ImGui::EndCombo();
	}
	auto& cpos2 = ImGui::GetCursorPos(); ImGui::SetCursorPos(cpos1); DrawTypeIcon(data.Container, data.Type); ImGui::SetCursorPos(cpos2);
	ImGui::SameLine();
	DrawTypeIcon(data.Container, data.Type);
...
void NodeEditorInternal::DrawGraphOption(NodeGraph& graph)
{
...
void NodeEditorInternal::UpdateSearchData()
{
    ...
    AddSearchData("Add Custom Event...", "Add Event", {}, {}, {}, false, &NodeEditorInternal::SpawnCustomEventNode);
    ...
    for (auto& graph : m_Graphs)
        for (auto& node : graph.Nodes)
            if (node.Function == NodeFunction::CustomEvent)
                AddSearchData(node.DisplayName, "Call Function", {}, {}, {}, false, &NodeEditorInternal::SpawnCallCustomEventNode, { (int)node.ID.Get() });

    for (auto& graph : m_Graphs)
        if (graph.Type == NodeGraphType::Macro)
            AddSearchData(graph.Name, "Macros", {}, {}, {}, false, &NodeEditorInternal::SpawnMacroNode, { graph.ID });

    for (auto& variable : m_Variables)
    {
		AddSearchData("Get " + variable.second.Name, "Variables", {}, {}, variable.second.Data, true, &NodeEditorInternal::SpawnGetVariableNode, { variable.second.ID });
		AddSearchData("Set " + variable.second.Name, "Variables", {}, { {{variable.second.Data }, PinKind::Input} }, variable.second.Data, false, &NodeEditorInternal::SpawnSetVariableNode, { variable.second.ID });
    }
}
...
void NodeEditorInternal::Application_Initialize()
{
    ...
    LoadNodeLibrary("src/Nodes/NodeLibrary.h");
    ...
    m_RemoveIcon = Dymatic::Texture2D::Create("assets/icons/SceneHierarchy/ClearIcon.png");
    ...
    CreateVariable("Test Bool", PinType::Bool).Data.Value.Bool = true;
    CreateVariable("Test Float", PinType::Float).Data.Value.Float = 5.423f;
    CreateVariable("Test Int", PinType::Int).Data.Value.Int = 12;
                    ...
                    ed::Begin(("Node Editor Panel" + std::to_string(window.ID)).c_str());
                    {
                        ...
                        for (auto& node : graph.Nodes)
                        {
                            ...
                            if (ImGui::IsItemClicked())
                            {
                                ...
                                {
                                    if (node.Function == NodeFunction::CallCustomEvent)
                        ...
                        if (pin)
                        {
                            ...
                            if (pin->Data.Type != PinType::Flow)
                                if (ImGui::MenuItem("Promote To Variable"))
                                {
                                    ...
                                    Node* node;
                                    if (in)
                                        node = SpawnGetVariableNode({ variable.ID });
                                    else
                                    {
		            				    node = SpawnSetVariableNode({ variable.ID });
        ...
        */
        ...
        // Script Contents Section
        {
...

            {
                ImGui::PushStyleVar(ImGuiStyleVar_FramePadding, ImVec2{ 4, 4 });
                float lineHeight = GImGui->Font->FontSize + GImGui->Style.FramePadding.y * 2.0f;
                bool open = ImGui::TreeNodeEx("##GraphsList", treeNodeFlags, "EVENT GRAPHS");
                ImGui::PopStyleVar();
                ImGui::SameLine(contentRegionAvailable.x - lineHeight * 0.5f);
                if (ImGui::Button("+", ImVec2{ lineHeight, lineHeight }))
                    CreateEventGraph();
...

            {
                ImGui::PushStyleVar(ImGuiStyleVar_FramePadding, ImVec2{ 4, 4 });
                float lineHeight = GImGui->Font->FontSize + GImGui->Style.FramePadding.y * 2.0f;
                bool open = ImGui::TreeNodeEx("##MacrosList", treeNodeFlags, "MACROS");
                ImGui::PopStyleVar();
                ImGui::SameLine(contentRegionAvailable.x - lineHeight * 0.5f);
                if (ImGui::Button("+", ImVec2{ lineHeight, lineHeight }))
                    CreateMacro();
		...
		// Details Section
        {
                ...
                if (m_Variables.find(m_SelectedDetailsReference) != m_Variables.end())
                {
                    auto& variable = m_Variables[m_SelectedDetailsReference];
                    {
                        ...
                        if (open)
                        {
                            ...
                            ImGui::NextColumn();
                            ImGui::Checkbox("##VariableInstanceEditable", &variable.InstanceEditable);
                            ImGui::Columns(1);
                ...
                for (auto& graph : m_Graphs)
                    for (auto& node : graph.Nodes)
                        if (node.ID == (ed::NodeId)m_SelectedDetailsReference)
                        {
                            {
                                ImGui::PushStyleVar(ImGuiStyleVar_FramePadding, ImVec2{ 4, 4 });
                                bool open = ImGui::TreeNodeEx("##EventInfo", treeNodeFlags, "EVENT");
                                ImGui::PopStyleVar();
                                if (open)
                                {
                                    ImGui::Columns(2);
...

                            {
                                ImGui::PushStyleVar(ImGuiStyleVar_FramePadding, ImVec2{ 4, 4 });
                                float lineHeight = GImGui->Font->FontSize + GImGui->Style.FramePadding.y * 2.0f;
                                bool open = ImGui::TreeNodeEx("##InputInfo", treeNodeFlags, "INPUTS");
                                ImGui::PopStyleVar();
                                ImGui::SameLine(contentRegionAvailable.x - lineHeight * 0.5f);
                                if (ImGui::Button("+", ImVec2{ lineHeight, lineHeight }))
            ...
            case NodeFunction::MacroBlock:
				if (auto macro = FindGraphByID(m_SelectedDetailsReference))
				{
					{
						ImGui::PushStyleVar(ImGuiStyleVar_FramePadding, ImVec2{ 4, 4 });
						bool open = ImGui::TreeNodeEx("##MacroInfo", treeNodeFlags, "MACRO");
						ImGui::PopStyleVar();
						if (open)
						{
							ImGui::Columns(2);
...

					{
						...
						for (auto& node : macro->Nodes)
							if (node.Function == NodeFunction::MacroBlock && node.Name == "Outputs")
							{
								ImGui::PushStyleVar(ImGuiStyleVar_FramePadding, ImVec2{ 4, 4 });
								float lineHeight = GImGui->Font->FontSize + GImGui->Style.FramePadding.y * 2.0f;
								bool open = ImGui::TreeNodeEx("##OutputInfo", treeNodeFlags, "OUTPUTS");
								ImGui::PopStyleVar();
								ImGui::SameLine(contentRegionAvailable.x - lineHeight * 0.5f);
								if (ImGui::Button("+", ImVec2{ lineHeight, lineHeight }))
...
Variable& NodeEditorInternal::CreateVariable(const std::string& name, PinType type)
{
	std::vector<std::string> vec;
	for (auto& variable : m_Variables)
		vec.push_back(variable.second.Name);
    int id = GetNextId();
    m_Variables[id] = { id, Dymatic::String::GetNextNameWithIndex(vec, name.empty() ? "NewVar" : name), (type == PinType::Void ? m_RecentPinType : type)};
...
void NodeEditorInternal::CreateEventGraph()
{
...
void NodeEditorInternal::CreateMacro()
{
    // Check for existing graph names
	std::vector<std::string> names;
	for (auto& graph : m_Graphs)
    ...
    // Setup input and output node
	{
		...
		input.Outputs.emplace_back(GetNextId(), "", PinType::Flow);
		...
		output.Inputs.emplace_back(GetNextId(), "", PinType::Flow);
...
void NodeEditorInternal::SetMacroName(int macroId, const std::string& name)
{
...
void NodeEditorInternal::AddMacroPin(int macroId, bool input)
{
    // Find macro
    if (auto macro = FindGraphByID(macroId))
    {
...
void NodeEditorInternal::UpdateMacroPin(int macroId, Pin* pin)
{
    if (auto macro = FindGraphByID(macroId))
    {
        size_t index = 0;
...
void NodeEditorInternal::RemoveMacroPin(int macroId, Pin* pin)
{
    if (auto macro = FindGraphByID(macroId))
    {
...
void NodeEditorInternal::SetCustomEventName(Node* event, const std::string& name)
{
    event->Name = name;
    ...
    for (auto& graph : m_Graphs)
        for (auto& node : graph.Nodes)
            if (node.Function == NodeFunction::CallCustomEvent && node.ReferenceId == event->ID.Get())
            {
                node.Name = name;
                node.DisplayName = name;
            }
...
void NodeEditorInternal::AddCustomEventInput(Node* event)
{
...
void NodeEditorInternal::UpdateCustomEventInput(Pin* input)
{
    size_t index = 0;
    ...
    if (index != input->Node->Outputs.size())
    {
        for (auto& graph : m_Graphs)
            for (auto& node : graph.Nodes)
                if (node.Function == NodeFunction::CallCustomEvent && node.ReferenceId == input->Node->ID.Get())
                {
