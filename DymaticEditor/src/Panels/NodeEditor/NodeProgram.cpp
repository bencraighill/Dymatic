...
//normalize angle, lerp angle, increment, decrement, equal, exp, division (whole and remainder), all basic opperators, floor fraction, hypotenuse, in range, int * float, lerp, map range clamped and unclamped, max, min, max array, min array, multiply by pi, nearly equal, normalize to range, search float for more
...
{
    std::string out = "//Dymatic C++ Include Script\r\r";
   
    out += "#include \"DymaticNodeLibrary.h\"\r\r";
	...
	std::ofstream fout("BlueprintCode.cpp");
    fout << out.c_str();
}
...
Dymatic::NodeCompilerLoopBackOutput Dymatic::NodeEditorPannel::CreateLoopBackVariables(int* inNode)
{
	...
	for (int i = 0; i < node->Inputs.size(); i++)
	{
        if (node->Inputs[i].Type != PinType::Flow)
        {
            ...
            {
                if (tempNode->Outputs[x].ID == tempPin->ID) { PinIndex = x; }
            }
