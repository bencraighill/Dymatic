...
namespace Dymatic {
	...
	void MaterialEditorInternal::CompileNodes()
	{
		...
		m_CompilerResults.push_back({ CompilerResultType::Info, "Build Started [Vulkan GLSL] - Dymatic Material Nodes Version " DY_VERSION_STRING });
		...
		compiler.WriteLine("// " DY_VERSION_COPYRIGHT ", All Rights Reserved");
		compiler.WriteLine("// Generated by Dymatic Material Node Editor Version " DY_VERSION_STRING);
		...
		if (errorCount == 0)
		{
			...
			m_CompilerResults.push_back({ CompilerResultType::Info, "Compile of 'Default Material' completed [Vulkan GLSL] - Dymatic Material Nodes Version " DY_VERSION_STRING });
		}
		else
			m_CompilerResults.push_back({ CompilerResultType::Info, "Compile of 'Default Material' failed [Vulkan GLSL] - " + std::to_string(errorCount) + " Error(s) " + std::to_string(warningCount) + " Warnings(s) - Dymatic Material Nodes Version " DY_VERSION_STRING });
	}
