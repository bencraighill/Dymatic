// Temporal Anti-Aliasing Resolution Shader
...
layout (set = 0, binding = 0) uniform sampler2D u_HistoryColor;
layout (set = 0, binding = 1) uniform sampler2D u_CurrentColor;
...
const bool u_IsNaiveTaa = false;
const float u_PreferAliasingOverBlur = 0.25; // Range from 0.0 to 1.0

void main()
{
    if (gl_GlobalInvocationID.x > u_ScreenDimensions.x || gl_GlobalInvocationID.y > u_ScreenDimensions.y)
        return;
    
    ivec2 imgCoord = ivec2(gl_GlobalInvocationID.xy);
    vec2 uv = (imgCoord + 0.5) / imageSize(u_Output);

    if (u_IsNaiveTaa)
    {
        vec2 velocity = texture(g_Velocity, uv).rg;
        vec2 historyUV = uv - velocity;

        vec3 currentColor = texture(u_CurrentColor, uv).rgb;
        vec3 historyColor = texture(u_HistoryColor, historyUV).rgb;

        float blend = 1.0 / u_TAASampleCount;

        vec3 color = mix(historyColor, currentColor, blend);
        imageStore(u_Output, imgCoord, vec4(color, 1.0));
    ...
    vec2 velocity = texture(g_Velocity, bestHistoryUv).rg;
    vec2 historyUV = uv - velocity;

    if (any(greaterThanEqual(historyUV, vec2(1.0))) || any(lessThan(historyUV, vec2(0.0))))
    {
        imageStore(u_Output, imgCoord, currentColor);
        return;
    }

    vec4 historyColor = SampleTextureCatmullRom(u_HistoryColor, historyUV);
    historyColor = clamp(historyColor, neighborhoodMin, neighborhoodMax);

    float blend = 1.0 / u_TAASampleCount;

    vec2 localPixelPos = fract(historyUV * textureSize(u_HistoryColor, 0));
    float pixelCenterDistance = abs(0.5 - localPixelPos.x) + abs(0.5 - localPixelPos.y);
    blend = mix(blend, 1.0, pixelCenterDistance * u_PreferAliasingOverBlur);
    
    vec4 color = mix(historyColor, currentColor, blend);
    imageStore(u_Output, imgCoord, color);
}
...
void GetResolveData(ivec2 imgCoord, out vec4 currentColor, out vec2 neighborhoodBestUv, out vec4 neighborhoodMin, out vec4 neighborhoodMax)
{
    ...
    neighborhoodMin = vec4(FLT_MAX);
    neighborhoodMax = vec4(FLT_MIN);
    for (int y = -1; y <= 1; y++)
    {
        for (int x = -1; x <= 1; x++)
        {
            ivec2 curPixel = imgCoord + ivec2(x, y);
            vec2 uv = (curPixel + 0.5) / imageSize(u_Output);

            vec4 neighborColor = texture(u_CurrentColor, uv);
            neighborhoodMin = min(neighborhoodMin, neighborColor);
            neighborhoodMax = max(neighborhoodMax, neighborColor);
            
            float inputDepth = texture(g_Depth, uv).r;
...
// This filter is better suited than the standard bilinear procedure as to not introducing too much of a blur
