...
void main()
{
    ...
    float focus = GetFocusScale(LinearDepth(texture(g_Depth, texCoord).r));
    ...
    for (int x = -2; x < 2; x++)
    {
        for (int y = -2; y < 2; y++)
        {
            avgColor += texture(g_Albedo, texCoord + (u_PixelSize * vec2(x, y))).xyz;
        }
    }

    avgColor /= 25.0;

    if (dot(avgColor - texture(g_Albedo, texCoord).xyz, vec3(0.2126, 0.7152, 0.0722)) > u_BokehThreshold)
    {
