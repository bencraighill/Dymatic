...
layout(std140, binding = USER_TEXTURE_SAMPLER_BUFFER_BINDING) uniform UserTextureSamplerBuffer
{
    uvec2 u_AlbedoMap;
};
...
void main()
{
	o_Albedo = vec4(texture(sampler2D(u_AlbedoMap), Input.TexCoord).rgb, 1.0);
    o_EntityID = u_EntityID;
    o_Normal = vec4(normalize(Input.Normal), 1.0);
	o_Emissive = vec4(0.0, 0.0, 0.0, 1.0);
    o_Roughness_Metallic_Specular_AO = vec4(0.2, 0.0, 1.0, 1.0);
}