...
void main()
{
    ...
    vec3 ambient = ambientStrength * vec3(texColor.x, texColor.y, texColor.z);
  	
    // diffuse 
    vec3 norm = normalize(Normal);
    vec3 lightDir = normalize(vec3(4.0, 4.0, 4.0) - v_Position);
    float diff = max(dot(norm, lightDir), 0.0);
    vec3 diffuse = diff * vec3(texColor.x, texColor.y, texColor.z);
            
    vec3 result = (ambient + diffuse) * vec3(texColor.x, texColor.y, texColor.z);
