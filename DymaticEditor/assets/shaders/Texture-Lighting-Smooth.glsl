...
void main()
{
	...
	float alphaX = v_TexCoord.x * 2.0;
	if (alphaX > 1.0)
	{
		alphaX = 2 - alphaX;
	}

	float alphaY = v_TexCoord.y * 2.0;
	if (alphaY > 1.0)
	{
		alphaY = 2 - alphaY;
	}
	...
	float distance = sqrt(pow((v_TexCoord.x - 0.5), 2) + pow((v_TexCoord.y - 0.5), 2));
	distance = (1 - distance);
	distance = (distance - 0.5) * 2.0;

	//result
    vec3 result = ambient + diffuse + specular;
    color = vec4(result.x, result.y, result.z, distance);
