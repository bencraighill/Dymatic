...
layout (set = 0, binding = 8) uniform sampler2D u_DepthMap;
...
vec3 CalculatePointLight(PointLight light, vec3 normal, vec3 fragPos, vec3 viewDir, vec3 albedoValue, vec3 specularValue, float shadow)
{
	...
	float attenuation = light.range /*1.0*/ / (LightConstant + LightLinear * distance + LightQuadratic * (distance * distance));

	//result
	vec3 ambient  = vec3(light.color * 0.1) * albedoValue;
	vec3 diffuse  = vec3(light.color * 0.8) * diff * albedoValue;
	vec3 specular = vec3(light.color * 1.0) * spec * specularValue;
	ambient  *= attenuation;
	diffuse  *= attenuation;
...
void main()
{
	...
	for (uint i = 0; i < lightCount; i++)
	{
		...
		result += CalculatePointLight(u_PointLights[lightVectorIndex], norm, Input.Position, viewDir, albedo, specular, shadow);
	}
